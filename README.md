...
可见MybatisPlus 提供更为丰富的方法，支持根据Map 查询更新 操作， 支持根据主键的批量操作, 支持分页,不必自己再去实现.

下面是实现的Service类提供的一些方法
...
原生方式
 ...
 Mybatis-plus 自己实现了分页查询,而且是物理分页, 物理分页就是在进行查询时带上了 limit 参数, 内存分页是指把所有的满足条件的数据查询出来然后在进行出来.
...
1. Mybatis-plus提供了很多基础的查询，对单表的简单查询也没必要重新在Mapper定义方法，然后在xml 文件写sql，直接在Service层进行操作
2. Mybatis-plus 提供了分页接口，而且是物理分页，不必在自己实现。
...
4. 3.0.7增加了lambda推导列名  不用很麻烦的写("xxxx"),也避免了输入出错的可能性。
   ...
   缺点可能是会存在一些Bug， Mapper层和Service层都继承了 mybatis-plus的类，对自己的扩展有一些限制。
...
2. SpringMvc分页需要在Mybatis的配置文件加入以下的配置
   <plugins>
    <!--
     | 分页插件配置
